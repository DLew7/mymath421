...
df3 = inner_join(df, df2, by = c('Country','date'))
head(df3)
...
df3 %>% 
  ...
  ggplot(aes(x = date,
           y = death_per_vac))+
  geom_point() +scale_y_continuous(trans='log') + labs(caption='The deaths per vaccine clearly went down starting from October 2022 until January, but then the deaths went up starting at the beginning of 2023.')
...
df3 %>% 
  ...
  ggplot(aes(x = date,
           y = death_per_vac))+
  geom_point() +scale_y_continuous(trans='log') + labs(caption='')
