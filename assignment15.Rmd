...
sum(is.na(df$workclass))
```


```{r}

df$workclass <- mode_impute(df$workclass)
sum(is.na(df$workclass))
...
d <- df %>% select(sex, hours.per.week)
viz(d)
...
d <- df %>% select(hours.per.week, sex)
viz(d)
```
...
viz <- function(d)
{
  ...
  else if (!is.numeric(d[[1]])&!is.numeric(d[[2]]))
  {
    ...
    d %>% ggplot(aes(x = d[[1]], fill = d[[2]]))+
      geom_bar(position='fill')+labs(x = names(d)[1], fill = names(d)[2])
...
d <- df %>% select(hours.per.week, capital.gain)
viz(d)
...
```{r}
d <- df %>% select(sex, marital.status)
viz(d)
```
