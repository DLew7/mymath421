---
title: "Video Games - checkboxGroupInput"
output: 
...
checkboxGroupInput(inputId = "decade_input", 
                   label = h3("Platform"),
...
renderPlotly({
df1 = df %>% 
  filter(Platform==input$decade_input) %>%
...
df3 = df %>% 
  ...
  geom_bar(mapping=aes(x=Year, fill=Platform), 
           position = 'dodge')+labs(title="Console Sales by Year")
ggplotly(df3)})
