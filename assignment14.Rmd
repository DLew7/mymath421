...
#df <- find_thread_urls(sort_by="new", subreddit = 'NHL')



```
```{r, eval=FALSE}
#write_csv(df, "reddit_NHL.csv")
```

```{r}

df = read_csv("reddit_NHL.csv")
head(df)
...
df$numComments = case_when(df$comments > 5 ~ "Long",
                         TRUE ~ "Short")
...
df %>%
    unnest_tokens(input = title, output = word) %>% 
    anti_join(get_stopwords()) %>% 
    count(numComments, word, sort = TRUE) %>%
    group_by(numComments) %>% 
    inner_join(get_sentiments("bing")) %>%
    filter(!is.na(sentiment)) %>%
    count(sentiment, sort = TRUE) %>% 
    group_by(numComments) %>% 
    mutate(n = n/sum(n)) %>% 
    ggplot(aes(numComments, n, fill=sentiment))+geom_col(position = 'dodge')+
    labs(y='Relative Frequency', x ='')
...
df$threads = case_when(df$comments > 5 ~ "Long",
                         TRUE ~ "Short")
...
#df <- find_thread_urls(sort_by="new", subreddit = 'funny')

```
```{r, eval=FALSE}
#write_csv(df, "reddit_funny.csv")
```
